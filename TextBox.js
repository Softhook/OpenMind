class TextBox {
          ...
          if (this.getStyledTextWidth(testLine) <= maxTextWidth) {
            ...
            if (currentLine) {
              ...
              for (let char of word) {
                if (this.getStyledTextWidth(charLine + char) <= maxTextWidth) {
                  charLine += char;
                } else {
  ...
  draw() {
    ...
    for (let i = 0; i < wrappedLines.length; i++) {
      ...
      // Render the line honoring **bold** markers
      const segments = TextBox.parseMarkdownBold(line);
      let x = textX;
      const y = startY + i * lineHeight;
      for (const seg of segments) {
        if (!seg || !seg.text) continue;
        textStyle(seg.bold ? BOLD : NORMAL);
        // Draw the segment and advance x by its visual width
        text(seg.text, x, y);
        x += this.getStyledTextWidth(seg.text);
      }
      // Reset style after line
      textStyle(NORMAL);
    }
